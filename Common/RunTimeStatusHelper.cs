...
namespace TaskTip.Common
{
    public static class RunTimeStatusHelper
    {
        ...
        public static string GetNetSpeedStatus()
        {
            ...
            return $"↑{uploadSpeed} {uploadUnit switch { 0 => "b/s", 1 => "Kb/s", 2 => "Mb/s", 3 => "Gb/s" }} | ↓{downloadSpeed} {downloadUnit switch { 0 => "b/s", 1 => "Kb/s", 2 => "Mb/s", 3 => "Gb/s" }}";
        }
        ...
        public static async Task<string> GetNetDownloadSpeedStatusAsync()
        {
            // 获取网络接口上传速率
            var downloadCounter = new PerformanceCounter("Network Interface", "Bytes Received/sec", "Intel[R] Wi-Fi 6 AX201 160MHz");
        ...
        public static async Task<string> GetNetUploadSpeedStatusAsync()
        {
            var uploadCounter = new PerformanceCounter("Network Interface", "Bytes Sent/sec", "Intel[R] Wi-Fi 6 AX201 160MHz");
            var uploadUnit = 0;
            var uploadSpeed = uploadCounter.NextValue();
            await Task.Delay(1000);
            uploadSpeed = uploadCounter.NextValue();
